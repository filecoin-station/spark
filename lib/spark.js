...
export default class Spark {
  ...
  async submitMeasurement (task, stats) {
    ...
    try {
      ...
      Zinnia.activity.info('Measurement submitted')
      return id
    } catch(err) {
      Zinnia.activity.error('Failed to submit measurement')
      throw err
    }
  ...
  async nextRetrieval () {
    ...
    Zinnia.activity.info('Testing retrieval...')
    const stats = {
      timeout: false,
      startAt: new Date(),
      firstByteAt: null,
    ...
    if (stats.timeout) {
      Zinnia.activity.error('Retrieval timed out')
    } else if (stats.carTooLarge) {
      Zinnia.activity.error('CAR too large')
    } else if (stats.statusCode >= 400) {
      Zinnia.activity.error(
        'The Storage Provider is not able to serve the retrieval'
      )
    } else {
      Zinnia.activity.info('Retrieval successful')
    }
