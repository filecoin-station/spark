...
export default class Spark {
  ...
  async executeRetrievalCheck (retrieval, stats) {
    const provider = await this.queryTheIndex(retrieval, stats)
    if (!provider) return
    stats.protocol = provider.protocol
    stats.providerAddress = provider.providerAddress
    ...
    const searchParams = new URLSearchParams({
      ...
      providers: provider.providerAddress
    })
  ...
  async queryTheIndex (retrieval, stats) {
    const url = `https://cid.contact/cid/${encodeURIComponent(retrieval.cid)}`

    let providerResults
    try {
      const res = await fetch(url)
      if (!res.ok) {
        stats.indexerResult = `ERROR_${res.status}`
        console.error('IPNI query failed:', url)
        console.error('HTTP response', res.status, await res.text())
        return
      }
      ...
      console.error('IPNI query failed:', url)
      console.error(err)
      stats.indexerResult = 'ERROR_FETCH'
      return
    }
    ...
    for (const p of providerResults) {
      ...
      console.log('Protocol:', protocolCode, protocol)
      ...
      switch (protocol) {
        case 'http':
          stats.indexerResult = 'OK'
          return { providerAddress, protocol }

        case 'graphsync':
          if (!graphsync) {
            graphsync = {
              providerAddress: `${providerAddress}/p2p/${p.Provider.ID}`,
              protocol
            }
          }
      }
    ...
    if (graphsync) {
      stats.indexerResult = 'HTTP_NOT_ADVERTISED'
      return graphsync
    }

    stats.indexerResult = 'NO_VALID_ADVERTISEMENT'
  }
